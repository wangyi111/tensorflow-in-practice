...
# YOUR CODE ENDS HERE
    ...
    num = len(list_all)
    num_train = round(num*SPLIT_SIZE)
    list_train = random.sample(list_all,num_train)
    print(list_train[:5])
    ...
    list_test = [x for x in list_all if x not in list_train]
    for fname in list_test:
        if os.path.getsize(os.path.join(SOURCE,fname)):
...
# YOUR CODE HERE
    ...
    tf.keras.layers.Dense(1,activation='sigmoid') # 0--cat, 1--dog
])
